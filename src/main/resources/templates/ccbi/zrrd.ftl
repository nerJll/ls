...
<body>
	...
	<div id='header' class="container-fluid">
		<div class="layui-row">
			...
			<button id='sendZRRD' onclick='sendZRRD()' type="button" class="btn btn-default">保存</button>
			</div>
		</div>
	</div>
		...
		<table id='tb2' class='table table-bordered'>
			...
			<#list 1..3 as i>
				<tr>
					<td>${i}</td>
					<td>
					<select name="zrrdZrbm" class="selectpicker form-control">
						<option></option>
						<option value='爱旭测试'>爱旭测试</option>
						<option value='爱旭测试1'>爱旭测试1</option>
						<option value='销售部'>销售部</option>
					</select>
					<td>
					<textarea rows="5" cols="9" name='zrrdZrr' class='form-control'></textarea>
					</td>
					<td>
					<textarea rows="5" cols="18" name='zrrdYl1' class='form-control'></textarea></td>
					<td>
					<textarea rows="5" cols="18" name='zrrdClyj' class='form-control' ></textarea></td>
					<td>
						<input id='sendZRBM' type='button' class='btn btn-primary' 
						onclick='send(this)' value='推送'>
					</td>
				</tr>
			</#list>
		 </table>
	...
	<script>
		...
		$(function(){
			...
			if('${aixuBaseLawsuit.lawsuitDanState}' == '阶段3-OA审批中'){	//若已提交WMS则不能再次提交
				...
				$("textarea[name='zrrdZrr']").each(function(){ 
				　　$(this).attr('readonly',true)
				})
				$("textarea[name='zrrdYl1']").each(function(){ 
				　　$(this).attr('readonly',true)
				})
			}
		...
		//推送部门经理
		function send(name){
			var zrbm = $(name).parents('tr').find('select[name=zrrdZrbm]').val()
			var zrr = $(name).parents('tr').find('textarea[name=zrrdZrr]').val()
			var yl1 = $(name).parents('tr').find('textarea[name=zrrdYl1]').val()
			var clyj = $(name).parents('tr').find('textarea[name=zrrdClyj]').val()
			if(zrbm == ''||zrr == ''||yl1 == ''||clyj == ''){
				alert('请填写完整信息')
			}else{
				var r = window.confirm("确认推送");
				if(r){
					/* alert('责任部门：' + zrbm + '\n' + '责任人：' + zrr + '\n' + 
							'调查意见：' + yl1 + '\n' + '处理意见：' + clyj) */
					$.ajax({
						url: '${ctx}/zrrd/sendZRBM',
						type: 'post',
						data: {
							'infoId': lawsuitId,
							'OAId': OAId,
						...
						success: function(data){
							alert(data.message)
							if(data.status == '1'){
								$(name).parents('tr').find('select[name=zrrdZrbm]').attr('disabled',true)
								$(name).parents('tr').find('textarea[name=zrrdZrr]').attr('readonly',true)
								$(name).parents('tr').find('textarea[name=zrrdYl1]').attr('readonly',true)
								$(name).parents('tr').find('textarea[name=zrrdClyj]').attr('readonly',true)
								$(name).parents('tr').find('#sendZRBM').attr('disabled',true)
							}
						},
						complete: function () {
					    	 $.bootstrapLoading.end()
		...
		function addTr1(){
			var html = '<tr>'
						+'<td>n</td>'
						+'<td>'
						+'<select name="zrrdZrbm" class="form-control">'
						+'<option></option>'
						+'<option value="爱旭测试">爱旭测试</option>'
						+'<option value="爱旭测试1">爱旭测试1</option>'
						+'<option value="销售部">销售部</option>'
